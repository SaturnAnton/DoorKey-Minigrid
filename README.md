...
Dai risultati ottenuti emerge che il numero di parametri, da solo, non è un indicatore sufficiente delle prestazioni su questo task. Il modello che ha ottenuto i risultati migliori in assoluto è `gpt-oss-120b`, che con il secondo prompt ha raggiunto il 100% di successo sia a livello di step (52/52) sia di episodi (4/4): un risultato che lo distingue nettamente da tutti gli altri modelli testati. Questo risultato è coerente con il fatto che `gpt-oss-120b` sia, tra quelli considerati, il modello con la maggiore capacità computazionale, oltre a essere presumibilmente ottimizzato per compiti che richiedono un ragionamento preciso e strutturato, come il calcolo di distanze e la comprensione di rappresentazioni spaziali dell'ambiente.

Di contro, `llama-3.3-70b-versatile`, pur essendo uno dei modelli con più parametri testati, ha ottenuto sistematicamente i risultati peggiori (0/52 step corretti con entrambi i prompt), mentre modelli più contenuti come `qwen3-32b` hanno raggiunto risultati nettamente superiori. Questo suggerisce che il fattore determinante non sia la dimensione del modello in sé, ma il tipo di addestramento ricevuto e la sua specializzazione nel ragionamento strutturato e nel calcolo spaziale, aspetti in cui modelli general-purpose come Llama risultano meno efficaci rispetto ad altri progettati o ottimizzati per compiti di logica e ragionamento step-by-step.

...
\textbf{esplorazione} e \textbf{sfruttamento}:
\begin{itemize}
  \item \textbf{Esplorazione}: consiste nel provare nuove azioni per scoprire
    nuove strategie che potrebbero portare a ricompense maggiori in futuro.

  \item \textbf{Sfruttamento}: consiste nel scegliere le azioni che si conoscono
    per massimizzare la ricompensa immediata.
\end{itemize}
La scelta dipende dal contesto e dall'obiettivo dell'agente.
Ci sono due approcci principali:
\begin{itemize}
  \item \textbf{\( \varepsilon \)-greedy}:
    La politica \( \varepsilon\)-greedy sceglie l'azione migliore di sempre, ma ogni tanto
    (con probabilità \( \varepsilon \)) sceglie un'azione a caso. Questo metodo è off-policy.
  \item \textbf{Softmax} (o Boltzmann):
    ...
    \begin{itemize}
      \item Quando la "temperatura" \( T \) è alta, tutte le azioni hanno probabilità simili
        di essere scelte (si esplora di più)
      \item Quando la temperatura \( T \) è bassa, l'azione con il valore \( Q(s,a) \)
        più alto ha una probabilità maggiore di essere scelta (si sfrutta di più)
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{\(\alpha\) - LEARNING RATE:} Parametro che determina il peso delle nuove informazioni rispetto a quelle passate; regola la velocità di aggiornamento dei valori nella Q-table.
    ...
    \item \textbf{\(\epsilon\) - EXPLORATION RATE:} Probabilità di scegliere un'azione casuale invece di quella ottimale stimata; fondamentale per esplorare l'ambiente.
\end{itemize}
...
Le azioni possibili all'interno di questo ambiente sono 7, identificate da un valore numerico:
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.4\textwidth]{figure/env.png}

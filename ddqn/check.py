...
def reward_llm(state, client, prompt, max_retries=8):
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"   [Parsing] Nessun numero valido trovato nell'output del VLM → reward = -0.005")
            return -0.005

        except Exception as e:
            if "429" in str(e) or "queue_exceeded" in str(e):
                wait = (2 ** attempt) + 60
                print(f"   [Rate limit Cerebras] tentativo {attempt+1}/{max_retries}: {e} — attendo {wait:.1f}s")
            else:
                wait = min(2 ** attempt, 5)
                print(f"   [Errore VLM] tentativo {attempt+1}/{max_retries}: {e} — attendo {wait}s")
            
            if attempt == max_retries - 1:
                print("   [Max retries] Cerebras non disponibile dopo i tentativi → reward = -0.005")
                return -0.005
...
def analizza_log_episodi(prompt_path, log_path):
    load_api_keys()
    client = CerebrasLLM(model_id="gpt-oss-120b")
